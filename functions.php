 ...
 */
function imax_wp_title( $title, $sep ) {
	global $paged, $page;
	...
	if ( $paged >= 2 || $page >= 2 )
		$title = "$title $sep " . sprintf( __( 'Page %s', 'i-max' ), max( $paged, $page ) );

	return $title;
}
add_filter( 'wp_title', 'imax_wp_title', 10, 2 );
...
function add_login_out_item_to_menu( $items, $args ){
	...
	if( is_user_logged_in( ) )
		$link = '<a href="' . wp_logout_url( $redirect ) . '" title="' .  __( 'Вийти' ) .'">' . __( 'Вийти' ) . '</a>';
